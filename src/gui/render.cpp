...
// Cache local para evitar acessos repetidos à memória
static Bitu cached_width;
...
// Otimização: usar memset em vez de loop quando possível
static void Check_Palette(void) {
	...
	case scalerMode8:
		GFX_SetPalette(render.pal.first, render.pal.last - render.pal.first + 1,
		               (GFX_PalEntry *)&render.pal.rgb[render.pal.first]);
		break;
	case scalerMode15:
	case scalerMode16:
...
// Otimização: cópia de cache com duff's device para melhor performance
static void RENDER_FinishLineHandler(const void * s) {
	if (s) {
		...
		// Duff's device para cópia rápida
		Bits n = (x + 7) / 8;
		switch (x % 8) {
		case 0:
			do {
...
static void RENDER_ClearCacheHandler(const void * src) {
	...
	// Preenchimento rápido com valores invertidos
	for (x = 0; x < width; x++)
...
static Bitu MakeAspectTable(Bitu skip, Bitu height, double scaley, Bitu miny) {
	...
	// Preencher zeros no início
	memset(&Scaler_Aspect[0], 0, skip * sizeof(Scaler_Aspect[0]));
...
static void RENDER_Reset(void) {
	...
	// Otimização: usar scaler mais simples quando possível
...
forcenormal:
	...
	case 15:
		...
		gfx_flags = (gfx_flags | GFX_LOVE_15 | GFX_RGBONLY) & ~GFX_CAN_8;
		break;
	case 16:
		render.src.start = (render.src.width * 2) / sizeof(Bitu);
		gfx_flags = (gfx_flags | GFX_LOVE_16 | GFX_RGBONLY) & ~GFX_CAN_8;
		break;
	case 32:
		render.src.start = (render.src.width * 4) / sizeof(Bitu);
		gfx_flags = (gfx_flags | GFX_LOVE_32 | GFX_RGBONLY) & ~GFX_CAN_8;
		break;
	}
	...
	// Configurar line handlers baseado no bpp
	switch (render.src.bpp) {
	case 8:
	...
	/* Reset the palette change detection */
	render.pal.first = 0;
	render.pal.last = 255;
	render.pal.changed = false;
	memset(render.pal.modified, 0, sizeof(render.pal.modified));
...
static bool RENDER_GetShader(std::string& shader_path, char *old_src) {
	...
	if (!fshader.is_open())
		fshader.open((shader_path + ".glsl").c_str(), std::ios_base::binary);
	...
	if (!buf.str().empty()) {
		...
		if (first_shell) {
			...
			for (Bitu i = 0; i < count; i++) {
				std::string env;
				if (!first_shell->GetEnvNum(i, env)) continue;
...
void RENDER_Init(Section * sec) {
	...
	render.scale.forced = (f == "forced");

	// Configuração de scaler otimizada - usar normal2x como padrão para performance
	if (scaler == "none") {
