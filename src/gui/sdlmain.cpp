...
#define DEFAULT_CONFIG_FILE "/NTVDBM.conf"
#elif defined(MACOSX)
#define DEFAULT_CONFIG_FILE "/Library/Preferences/NTVDBM Preferences"
#else
#define DEFAULT_CONFIG_FILE "/.NTVDBMrc"
#endif
...
static void GUI_StartUp(Section * sec) {
	...
	GFX_Stop();
	SDL_WM_SetCaption("NTVDBM",VERSION);
...
#if C_DEBUG
#else
	MAPPER_AddHandler(&PauseDOSBox, MK_pause, MMOD2, "pause", "Pause NTVDBM");
#endif
...
void Config_Add_SDL() {
	...
	//Pbool->Set_help("Start NTVDBM directly in fullscreen.");
	...
	//Pbool->Set_help("Wait before closing if NTVDBM has an error.");

	Pmulti = sdl_sec->Add_multi("priority", Property::Changeable::Always, ",");
	Pmulti->SetValue("higher,normal");
	//Pmulti->Set_help("Priority levels for NTVDBM.");
...
static void launchcaptures(std::string const& edit) {
	std::string path,file;
	Section* t = control->GetSection("NTVDBM");
...
static void eraseconfigfile() {
	FILE* f = fopen("NTVDBM.conf","r");
	if(f) {
		fclose(f);
		show_warning("Warning: NTVDBM.conf exists in current working directory.\nThis will override the configuration file at runtime.\n");
	}
	std::string path,file;
...
static void erasemapperfile() {
	FILE* g = fopen("NTVDBM.conf","r");
	if(g) {
		fclose(g);
		show_warning("Warning: NTVDBM.conf exists in current working directory.\nKeymapping might not be properly reset.\n"
		             "Please reset configuration as well and delete the NTVDBM.conf.\n");
	}
...
int main(int argc, char* argv[]) {
	...
	try {
		...
		if(!control->configfiles.size()) control->ParseConfigFile("NTVDBM.conf");
