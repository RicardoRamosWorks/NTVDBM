...
bool ConnectToServer(char const *strAddr) {
	...
	if(!SDLNet_ResolveHost(&ipxServConnIp, strAddr, (Bit16u)udpPort)) {
		...
		if(ipxClientSocket) {
			...
			L//OG_MSG("IPX: Unable to open socket");
		}
	} else {
...
public:
	void HelpCommand(const char *helpStr) {
		...
		if(strcasecmp("connect", helpStr) == 0) {
			...
			WriteOut("NTVDBM session.  The \"address\" parameter specifies the IP address or host name\n");
		...
		if(strcasecmp("startserver", helpStr) == 0) {
			WriteOut("IPXNET STARTSERVER starts and IPX tunneling server on this NTVDBM session.  By\n");
			WriteOut("default, the server will accept connections on UDP port 213, though this can be\n");
			WriteOut("changed.  Once the server is started, NTVDBM will automatically start a client\n");
		...
		if(strcasecmp("stopserver", helpStr) == 0) {
			WriteOut("IPXNET STOPSERVER stops the IPX tunneling server running on this NTVDBM\nsession.");
		...
		if(strcasecmp("status", helpStr) == 0) {
			WriteOut("IPXNET STATUS reports the current state of this NTVDBM's sessions IPX tunneling\n");
	...
	void Run(void)
	{
		WriteOut("IPX Tunneling utility for NTVDBM\n\n");
		if(!cmd->GetCount()) {
