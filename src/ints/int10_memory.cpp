...
void INT10_LoadFont(PhysPt font,bool reload,Bitu count,Bitu offset,Bitu map,Bitu height) {
	...
	IO_Write(0x3c4,0x02);
	IO_Write(0x3c5,0x04); // select plane 2 for writing
	IO_Write(0x3c4,0x04);
	IO_Write(0x3c5,0x07); // odd/even off in SEQ
	IO_Write(0x3ce,0x04);
	IO_Write(0x3cf,0x02); // select plane 2 for reading
	IO_Write(0x3ce,0x05);
	IO_Write(0x3cf,0x00); // write mode 0, odd/even off in GFX
	IO_Write(0x3ce,0x06);
	IO_Write(0x3cf,0x04); // CPU memory window A0000-AFFFF
	...
	IO_Write(0x3c4,0x02);
	IO_Write(0x3c5,0x03); // select planes 0&1 for writing
	IO_Write(0x3c4,0x04);
	IO_Write(0x3c5,0x03); // odd/even on in SEQ
	IO_Write(0x3ce,0x04);
	IO_Write(0x3cf,0x00); // select plane 0 for reading
	IO_Write(0x3ce,0x05);
	IO_Write(0x3cf,0x10); // write mode 0, odd/even on in GFX
	IO_Write(0x3ce,0x06);
	IO_Write(0x3cf,mono?0x0a:0x0e); // Bx000-BxFFF, odd/even on
...
void INT10_SetupRomMemory(void) {
	...
	if (IS_EGAVGA_ARCH) {
		...
		if (IS_VGA_ARCH) {
			...
			phys_writew(rom_base+int10.rom.used,0x0000);
			int10.rom.used+=2;
			phys_writew(rom_base+int10.rom.used,0x0100);
			int10.rom.used+=2;
			phys_writew(rom_base+int10.rom.used,0x0200);
			int10.rom.used+=2;
			phys_writew(rom_base+int10.rom.used,0x0102);
			int10.rom.used+=2;
			phys_writew(rom_base+int10.rom.used,0x0400);
			int10.rom.used+=2;
			phys_writew(rom_base+int10.rom.used,0x0104);
			int10.rom.used+=2;
			phys_writew(rom_base+int10.rom.used,0x0500);
			int10.rom.used+=2;
			phys_writew(rom_base+int10.rom.used,0x0502);
			int10.rom.used+=2;
			phys_writew(rom_base+int10.rom.used,0x0600);
			int10.rom.used+=2;
			phys_writew(rom_base+int10.rom.used,0x0601);
			int10.rom.used+=2;
			phys_writew(rom_base+int10.rom.used,0x0605);
			int10.rom.used+=2;
			phys_writew(rom_base+int10.rom.used,0x0800);
			int10.rom.used+=2;
			phys_writew(rom_base+int10.rom.used,0x0801);
			int10.rom.used+=2;
			phys_writew(rom_base+int10.rom.used,0x0700);
			int10.rom.used+=2;
			phys_writew(rom_base+int10.rom.used,0x0702);
			int10.rom.used+=2;
			phys_writew(rom_base+int10.rom.used,0x0706);
			int10.rom.used+=2;
			...
			int10.rom.used+=4;
			phys_writed(rom_base+int10.rom.used,0);
			int10.rom.used+=4;
			phys_writed(rom_base+int10.rom.used,0);
			int10.rom.used+=4;
			phys_writed(rom_base+int10.rom.used,0);
