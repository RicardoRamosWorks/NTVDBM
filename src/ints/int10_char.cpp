...
static void TANDY16_CopyRow(Bit8u cleft,Bit8u cright,Bit8u rold,Bit8u rnew,PhysPt base) {
	...
	Bitu copy=(cright-cleft)*4;
	Bitu nextline=CurMode->twidth*4;
...
static void EGA16_CopyRow(Bit8u cleft,Bit8u cright,Bit8u rold,Bit8u rnew,PhysPt base) {
	...
	IO_Write(0x3ce,5);
	IO_Write(0x3cf,1);		/* Memory transfer mode */
	IO_Write(0x3c4,2);
...
void INT10_ScrollWindow(Bit8u rul,Bit8u cul,Bit8u rlr,Bit8u clr,Bit8s nlines,Bit8u attr,Bit8u page) {
	...
	while (start!=end) {
		...
		case M_CGA4:
			...
			break;
		case M_TANDY16:
			TANDY16_CopyRow(cul,clr,start,start+nlines,base);
			break;
		case M_EGA:
...
filling:
	...
	for (; nlines>0; nlines--) {
		...
		case M_CGA4:
			...
			break;
		case M_TANDY16:
			TANDY16_FillRow(cul,clr,start,base,attr);
			break;
		case M_EGA:
...
void INT10_SetActivePage(Bit8u page) {
	...
	if (svgaCard==SVGA_S3Trio) page &= 7;
