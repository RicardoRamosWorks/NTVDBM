...
void PROGRAMS_MakeFile(char const *const name, PROGRAMS_Main *main) {
	...
	if (internal_progs.size() > 255)
		E_Exit("PROGRAMS_MakeFile program size too large (%d)",
		       static_cast<int>(internal_progs.size()));
...
void Program::ChangeToLongCmd() {
	 ...
	 * it is also done for long arguments to as it is convient (as the total
	 * commandline can be longer then 127 characters. imgmount with lot's of
	 * parameters Length of arguments can be ~120. but switch when above 100 to
	 * be sure
	 */

	if (/*control->SecureMode() ||*/ cmd->Get_arglength() > 100) {
		CommandLine *temp =
		    new CommandLine(cmd->GetFileName(), full_arguments.c_str());
		delete cmd;
		cmd = temp;
	}
...
bool Program::SetEnv(const char *entry, const char *new_string) {
	...
	if (new_string[0]) {
		...
		for (std::string::iterator it = bigentry.begin(); it != bigentry.end();
		        ++it)
			*it = toupper(*it);
...
void CONFIG::Run(void) {
	static const char *const params[] = {
		"-r",           "-wcp",    "-wcd",    "-wc",        "-writeconf",
		"-l",           "-rmconf", "-h",      "-help",      "-?",
		"-axclear",     "-axadd",  "-axtype", "-avistart",  "-avistop",
		"-startmapper", "-get",    "-set",    "-writelang", "-wl",
		"-securemode",  ""
	};
	enum prs {
		P_NOMATCH,
	...
	while (presult != P_NOPARAMS) {
		...
		case P_WRITECONF2:
			...
			if (pvars.size() > 1)
				...
				// write config to specific file, except if it is an absolute
				// path
		...
		case P_WRITECONF_PORTABLE:
			...
			if (pvars.size() > 1)
				...
				// -wcp without parameter: write dosbox.conf to startup
				// directory
				if (control->configfiles.size())
					writeconf(std::string("NTVDBM.conf"), false);
				else
					WriteOut(MSG_Get("PROGRAM_CONFIG_NOCONFIGFILE"));
			}
			break;
		...
		case P_HELP3: {
			...
			case 2: {
				...
				Section *sec = control->GetSection(pvars[0].c_str());
				Section *sec2 =
				    control->GetSectionFromProperty(pvars[1].c_str());
				if (sec != sec2) {
					WriteOut(MSG_Get("PROGRAM_CONFIG_PROPERTY_ERROR"));
				}
				break;
			...
			if (psec == NULL) {
				...
				WriteOut(
				    MSG_Get("PROGRAM_CONFIG_HLP_LINEHLP"), pline->GetName(),
				    // this is 'unclean' but the autoexec section has no help
				    // associated
				    MSG_Get("AUTOEXEC_CONFIGFILE_HELP"), pline->data.c_str());
				return;
			}
			if (pvars.size() == 1) {
				size_t i = 0;
				...
				while (true) {
					...
					if (!strcasecmp(p->propname.c_str(), pvars[1].c_str())) {
						...
						for (Bitu k = 0; k < pv.size(); k++) {
							if (pv[k].ToString() == "%u")
								propvalues +=
								    MSG_Get("PROGRAM_CONFIG_HLP_POSINT");
							else
		...
		case P_SETPROP: {
			...
			if ((equpos != std::string::npos) &&
				...
				Section *sec =
				    control->GetSectionFromProperty(pvars[0].c_str());
				if (sec)
					pvars.insert(pvars.begin(), std::string(sec->GetName()));
				else {
					WriteOut(MSG_Get("PROGRAM_CONFIG_PROPERTY_ERROR"));
				...
				if (!sec) {
					...
					// first of pvars is most likely a section, but could still
					// be gus have a look at the second parameter
					if (pvars.size() < 2) {
						WriteOut(MSG_Get("PROGRAM_CONFIG_SET_SYNTAX"));
						return;
					}
					std::string::size_type spcpos2 =
					    pvars[1].find_first_of(' ');
					std::string::size_type equpos2 =
					    pvars[1].find_first_of('=');
					if ((equpos2 != std::string::npos) &&
					        ((spcpos2 == std::string::npos) ||
					         (equpos2 < spcpos2))) {
						// split on the =
			...
			while (value.size() && (value.at(0) == ' ' || value.at(0) == '='))
				value.erase(0, 1);
			...
			if (!change_success)
				WriteOut(MSG_Get("PROGRAM_CONFIG_VALUE_ERROR"), value.c_str(),
				         pvars[1].c_str());
...
void PROGRAMS_Init(Section * /*sec*/) {
	...
	MSG_Add("PROGRAM_CONFIG_CONFDIR",
	        "NTVDBM %s configuration directory: \n%s\n\n");
	...
	MSG_Add(
	    ...
	    "Config tool:\n"
	    "-writeconf or -wc without parameter: write to primary loaded config "
	    "file.\n"
	    "-writeconf or -wc with filename: write file to config directory.\n"
	    "Use -writelang or -wl filename to write the current language "
	    "strings.\n"
	    "-r [parameters]\n Restart NTVDBM, either using the previous "
	    "parameters or any that are appended.\n"
	    "-wcp [filename]\n Write config file to the program directory, "
	    "NTVDBM.conf or the specified \n filename.\n"
	    ...
	    " Without parameters, displays this help screen. Add \"sections\" for "
	    "a list of\n sections."
	...
	MSG_Add("PROGRAM_CONFIG_HLP_PROPHLP",
	        "Purpose of property \"%s\" (contained in section "
	        "\"%s\"):\n%s\n\nPossible Values: %s\nDefault value: %s\nCurrent "
	        "value: %s\n");
	...
	MSG_Add("PROGRAM_CONFIG_HLP_SECTLIST",
	        "NTVDBM configuration contains the following sections:\n\n");
	...
	MSG_Add(
	    ...
	    "\nNTVDBM was started with the following command line parameters:\n%s");
