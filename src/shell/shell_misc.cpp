...
void DOS_Shell::ShowPrompt(void) {
	...
	dir[0] = 0; // DOS_GetCurrentDir doesn't always return something. (if drive
	// is messed up)
	...
	while (size) {
		...
		case 0x00: /* Extended Keys */
		{
			...
			case 0x3d: /* F3 */
				...
				if (it_history != l_history.end() &&
				        it_history->length() > str_len) {
			...
			case 15: /* Shift-Tab */
				if (l_completion.size()) {
					...
					if (it_completion->length()) {
						...
						size = CMD_MAXLINE - str_index - 2;
						DOS_WriteFile(STDOUT, (Bit8u *)it_completion->c_str(),
						              &len);
					}
				}
			default:
		...
		case '\t': {
			if (l_completion.size()) {
				...
				if (p_completion_start) {
					p_completion_start++;
					completion_index =
					    (Bit16u)(str_len - strlen(p_completion_start));
				} else {
				...
				if (p_completion_start) {
					...
					// not perfect when line already contains wildcards, but
					// works
					if ((dot_pos - bs_pos > 0) && (dot_pos - fs_pos > 0) &&
					        (dot_pos - cl_pos > 0))
				...
				while (res) {
					...
					if (strcmp(name, ".") && strcmp(name, "..")) {
						if (dir_only) { // Handle the dir only case different
							// (line starts with cd)
							if (att & DOS_ATTR_DIRECTORY)
								l_completion.push_back(name);
						} else {
							ext = strrchr(name, '.');
							if (ext && (strcmp(ext, ".BAT") == 0 ||
							            strcmp(ext, ".COM") == 0 ||
							            strcmp(ext, ".EXE") == 0))
								// we add executables to the a seperate list and
								// place that list infront of the normal files
				...
				std::copy(executable.begin(), executable.end(),
		...
		case 0x1b: /* ESC */
			...
			*line = 0; // reset the line.
			if (l_completion.size())
				l_completion.clear(); // reset the completion list.
		...
		default:
			if (l_completion.size())
				l_completion.clear();
			if (str_index < str_len &&
			        true) {    // mem_readb(BIOS_KEYBOARD_FLAGS1)&0x80) dev_con.h ?
				...
				line[++str_len] = 0; // new end (as the internal buffer moved
...
bool DOS_Shell::Execute(char *name, char *args) {
	...
	/*only internal commands can be run this way and they never get in this
	 * handler */
	if (extension == 0) {
	...
	if (strcasecmp(extension, ".bat") == 0) { /* Run the .bat file */
		...
		for (char *p = parseline; (p - parseline) < 250;
		        p++) { // Stay relaxed within boundaries as we have plenty of room
			if (*p == '/') { // Transform /Hello into H\0ello
				*p = 0;
				p++;
				while (*p == 0 && (p - parseline) < 250)
					p++;                     // Skip empty fields
				if ((p - parseline) < 250) { // Found something. Lets get the
					// first letter and break it up
					p++;
					memmove(static_cast<void *>(p + 1), static_cast<void *>(p),
					        (250 - (p - parseline)));
					if ((p - parseline) < 250)
						*p = 0;
				}
		...
		parseline[255] = parseline[256] = parseline[257] = 0; // Just to be
		// safe.
		...
		Bit8u add;
		Bit16u skip = 0;
		// find first argument, we end up at parseline[256] if there is only one
		// argument (similar for the second), which exists and is 0.
		while (skip < 256 && parseline[skip] == 0)
			skip++;
		FCB_Parsename(dos.psp(), 0x5C, 0x01, parseline + skip, &add);
		skip += add;

		// Move to next argument if it exists
		while (parseline[skip] != 0)
			skip++; // This is safe as there is always a 0 in parseline at the
		// end.
		while (skip < 256 && parseline[skip] == 0)
			skip++; // Which is higher than 256
...
char *DOS_Shell::Which(char *name) {
	...
	while (*pathenv) {
		...
		if (size_t len = strlen(path)) {
			...
			if ((name_len + len + 1) >= DOS_PATHLENGTH)
				continue;
			strcat(path, name);
