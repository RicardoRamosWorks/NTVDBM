...
using namespace std;
static std::string current_config_dir; // Set by parseconfigfile so Prop_path
// can use it to construct the realpath
...
bool Property::CheckValue(Value const &in, bool warn) {
	...
	// might now be reset to the default value:
	// %s",in.ToString().c_str(),propname.c_str(),default_value.ToString().c_str());
	return false;
}

void Property::Set_help(string const & /*in*/) {
	// Não faz nada: o help agora é estático e gerenciado via template embutido
}
...
bool Prop_int::SetVal(Value const &in, bool forced, bool warn) {
	if (forced) {
		...
		// if (warn) LOG_MSG("%s is outside the allowed range %s-%s for
		// variable: %s.\nIt has been set to the closest boundary:
		// %d.",in.ToString().c_str(),min.ToString().c_str(),max.ToString().c_str(),propname.c_str(),va);
...
bool Prop_int::CheckValue(Value const &in, bool warn) {
	//	if(!suggested_values.empty() && Property::CheckValue(in,warn)) return
	//true;
	if (!suggested_values.empty())
	...
	// might now be reset to the default value:
	// %s",in.ToString().c_str(),min.ToString().c_str(),max.ToString().c_str(),propname.c_str(),default_value.ToString().c_str());
	return false;
}
...
bool Prop_multival::SetValue(std::string const &input) {
	...
	while ((p = section->Get_prop(i++))) {
		...
		if (p->Get_type() == Value::V_STRING) {
			...
			// Non-strings can have more things, conversion alone is not enough
			// (as invalid values as converted to 0)
			bool r = p->SetValue(in);
			if (!r) {
				if (in.empty() && p->Get_type() == prevtype) {
					// Nothing there, but same type of variable, so repeat it
					// (sensitivity)
					in = prevargument;
					p->SetValue(in);
				} else {
					// Something was there to be parsed or not the same type.
					// Invalidate entire property.
					make_default_value();
				}
			}
		}
...
Prop_int *Section_prop::Add_int(string const &_propname,
...
Prop_string *Section_prop::Add_string(string const &_propname,
...
Prop_path *Section_prop::Add_path(string const &_propname,
                                  Property::Changeable::Value when,
                                  char const *const _value) {
...
Prop_bool *Section_prop::Add_bool(string const &_propname,
                                  Property::Changeable::Value when,
                                  bool _value) {
...
Prop_hex *Section_prop::Add_hex(string const &_propname,
...
Prop_multival *Section_prop::Add_multi(std::string const &_propname,
                                       Property::Changeable::Value when,
...
Section_prop::Add_multiremain(std::string const &_propname,
                              Property::Changeable::Value when,
                              std::string const &sep) {
...
Prop_multival_remain *
Section_prop::Get_multivalremain(string const &_propname) const {
	for (const_it tel = properties.begin(); tel != properties.end(); ++tel) {
		if ((*tel)->propname == _propname) {
			Prop_multival_remain *val =
			    dynamic_cast<Prop_multival_remain *>((*tel));
			if (val)
				return val;
			else
				return NULL;
...
void Section_prop::PrintData(FILE *outfile) const {
	...
	// fprintf wants int instead of size_t, also refuse to print sections with
	// weirdly long properties.
	int intlen = (len > 100) ? 100 : static_cast<int>(len);
	if (intlen == 100)
		return;
	for (const_it tel = properties.begin(); tel != properties.end(); ++tel) {
		fprintf(outfile, "%-*s = %s\n", intlen, (*tel)->propname.c_str(),
...
bool Config::PrintConfig(char const *const configfilename) const {
	...
	// Template oficial embutido no executável com os placeholders de seções
	static const std::string template_text = R"CONF_TEMPLATE(#
#			███╗   ██╗████████╗
#			████╗  ██║╚══██╔══╝
#			██╔██╗ ██║   ██║					NT Virtual DosBox Machine
#			██║╚██╗██║   ██║					by  RicardoRamosWorks.com
#			██║ ╚████║   ██║
#			╚═╝  ╚═══╝   ╚═╝
#											   A replacement for NTVDBM on
#   ██╗   ██╗██████╗ ██████╗ ███╗   ███╗		  unsupported machines
#   ██║   ██║██╔══██╗██╔══██╗████╗ ████║
#   ██║   ██║██║  ██║██████╔╝██╔████╔██║
#   ╚██╗ ██╔╝██║  ██║██╔══██╗██║╚██╔╝██║	    Optimized for AppleTV 1st
#    ╚████╔╝ ██████╔╝██████╔╝██║ ╚═╝ ██║			   version 1.0
#     ╚═══╝  ╚═════╝ ╚═════╝ ╚═╝     ╚═╝
# 
#		This is the emulator configuration file; you can configure the virtual computer
# according to your needs. Lines starting with # are comments and will be ignored by NTVDBM.
#	   Please use Notepad++ or equivalent for the best display of characters and tabs.

[sdl]
#  ╔══════════════════════╤══════════════════════════════════════════════════════════════╗
#  ║    ☻ fullscreen ☻    │ Start NTVDBM directly in fullscreen                          ║
#  ╟──────────────────────┼──────────────────────────────────────────────────────────────╢
#  ║    ☻ fulldouble ☻    │ Use double buffering in fullscreen                           ║
#  ╟──────────────────────┼──────────────────────────────────────────────────────────────╢
#  ║  ☻ fullresolution ☻  │ What resolution to use for fullscreen                        ║
#  ╟──────────────────────┼──────────────────────────────────────────────────────────────╢
#  ║ ☻ windowresolution ☻ │ Scale the window to this size                                ║
#  ╟──────────────────────┼──────────────────────────────────────────────────────────────╢
#  ║      ☻ output ☻      │ What video system to use for output                          ║
#  ║                      │ Values: surface, overlay, opengl, openglnb                   ║
#  ╟──────────────────────┼──────────────────────────────────────────────────────────────╢
...
#  ╟──────────────────────┼──────────────────────────────────────────────────────────────╢
#  ║     ☻ priority ☻     │ Priority levels for NTVDBM                                   ║
#  ║                      │ Values: [lowest] [lower] [normal] [higher] [highest] [pause] ║
#  ╟──────────────────────┼──────────────────────────────────────────────────────────────╢
...
#	║					│ of the NTVDBM configuration directory, or one of the built-in shaders:						║
#	║					│ [advinterp2x] [advmame2x] [rgb2x] [scan2x] [tv2x] [sharp]										║
#	╚═══════════════════╧═══════════════════════════════════════════════════════════════════════════════════════════════╝
...
#	╟───────────────────┼───────────────────────────────────────────────────────────────────────────────╢
#	║					│ Number of instructions NTVDBM tries to emulate each millisecond				║
#	║					│ Setting this value too high results in sound dropouts and lags				║
#	║					│ Cycles can be set in 3 ways:													║
#	║					│	'auto'			tries to guess what a game needs							║
#	║	  ≡ cycles ≡	│					It usually works, but can fail for certain games			║
#	║					│	'fixed #number' will set a fixed number of cycles. This is what you usually	║
#	║					│					need if 'auto' fails (Example: fixed 4000)					║
#	║					│	'max'			will allocate as much cycles as your computer is able to	║
#	║					│					handle														║
#	║					│ Values: [auto] [fixed] [max ]													║
#	╟───────────────────┼───────────────────────────────────────────────────────────────────────────────╢
...
[sblaster]
#	╔════════════════╤══════════════════════════════════════════════════════════════════════════════════════════════════════╗
#	║   ♫ sbtype ♫   │ Type of Soundblaster to emulate																		║
#	║                │ Values: [sb1] [sb2] [sbpro1] [sbpro2] [sb16] [none]													║
#	╟────────────────┼──────────────────────────────────────────────────────────────────────────────────────────────────────╢
...
#	║                │ Values: [1] [5] [0] [3] [6] [7]																		║
#	╟────────────────┼──────────────────────────────────────────────────────────────────────────────────────────────────────╢
#	║   ♫ sbmixer ♫  │ Allow the soundblaster mixer to modify the NTVDBM mixer												║
#	╟────────────────┼──────────────────────────────────────────────────────────────────────────────────────────────────────╢
#	║   ♫ oplmode ♫  │ Type of OPL emulation. On 'auto' the mode is determined by sblaster type								║
#	║                │ Values: [auto] [cms] [opl2] [dualopl2] [opl3] [opl3gold] [none]										║
#	╟────────────────┼──────────────────────────────────────────────────────────────────────────────────────────────────────╢
...
#	║  ↔ joysticktype ↔	│	4axis_2 (supports one joystick, second joystick used),								║
#	║					│	fcs (Thrustmaster), ch (CH Flightstick)												║
#	║					│	auto chooses emulation depending on real joystick(s)								║
#	║					│	(Remember to reset NTVDBM's mapperfile if you saved it earlier)						║
...
#	║	  ☼ ems ☼	 │	other choices, or require EMS support to be disabled (=false)	 ║
#	║				 │	to work at all													 ║
#	║				 │	Values: true, emsboard, emm386, false							 ║
#	╟────────────────┼───────────────────────────────────────────────────────────────────╢
...
{{AUTOEXEC_VALUES}}
    ...
    std::string final_config = template_text;

    // Varre todas as seções registradas para processar seus dados e substituir
    // no template
    for (const_it tel = sectionlist.begin(); tel != sectionlist.end(); ++tel) {
        Section *sec = *tel;
        std::string sec_name = sec->GetName();

        // Monta a string do placeholder padrão, ex: {{SDL_VALUES}}
        std::string placeholder = "{{" + sec_name + "_VALUES}}";
        for (size_t k = 0; k < placeholder.length(); ++k) {
            if (placeholder[k] >= 'a' && placeholder[k] <= 'z') {
                placeholder[k] -= 32; // Transforma em uppercase
            }
        }

        std::string sec_values = "";

        // Se for uma seção estruturada de propriedades (Propriedades normais)
        Section_prop *sec_prop = dynamic_cast<Section_prop *>(sec);
        if (sec_prop) {
            size_t max_len = 0;
            int idx = 0;

            // Loop utilizando a estrutura interna exposta por Get_prop para
            // pegar a largura máxima de alinhamento
            while (Property *p = sec_prop->Get_prop(idx++)) {
                if (p->propname.length() > max_len) {
                    max_len = p->propname.length();
                }
            }

            int intlen = (max_len > 100) ? 100 : static_cast<int>(max_len);
            if (intlen < 100) {
                idx = 0;
                while (Property *p = sec_prop->Get_prop(idx++)) {
                    char buf[512];
                    // Formatação idêntica e perfeitamente alinhada com espaços
                    // baseada no item mais longo da seção
            ...
            // Se for uma seção baseada em linhas de texto puro (como o
            // [autoexec])
            Section_line *sec_line = dynamic_cast<Section_line *>(sec);
            if (sec_line) {
                sec_values = sec_line->data;
            }
        }

        // Realiza a substituição da tag correspondente pela lista de
        // configurações reais
        size_t start_pos = 0;
        while ((start_pos = final_config.find(placeholder, start_pos)) !=
               std::string::npos) {
            final_config.replace(start_pos, placeholder.length(), sec_values);
            start_pos += sec_values.length();
        }
    }

    // Varredura de segurança: apaga qualquer placeholder que sobrou por seções
    // não instanciadas
    size_t start_pos = 0;
    while ((start_pos = final_config.find("{{")) != std::string::npos) {
        size_t end_pos = final_config.find("}}", start_pos);
        if (end_pos != std::string::npos) {
            final_config.erase(start_pos, (end_pos + 2) - start_pos);
        } else {
            break;
        }
    }

    // Cospe todo o resultado de texto mesclado diretamente no arquivo conf de
    // saída
    fputs(final_config.c_str(), outfile);
    fclose(outfile);
    return true;
}

Section_prop *Config::AddSection_prop(char const *const _name,
                                      void (*_initfunction)(Section *),
                                      bool canchange) {
...
Section_prop::~Section_prop() {
    // ExecuteDestroy should be here else the destroy functions use destroyed
    // properties
    ExecuteDestroy(true);
...
Section_line *Config::AddSection_line(char const *const _name,
                                      void (*_initfunction)(Section *)) {
...
void Section::ExecuteInit(bool initall) {
    ...
    for (func_it tel = initfunctions.begin(); tel != initfunctions.end();
         ++tel) {
...
void Section::ExecuteDestroy(bool destroyall) {
    typedef std::list<Function_wrapper>::iterator func_it;
    for (func_it tel = destroyfunctions.begin();
         tel != destroyfunctions.end();) {
        if (destroyall || (*tel).canchange) {
            (*tel).function(this);
            tel =
                destroyfunctions.erase(tel); // Remove destroyfunction once used
        } else
            ++tel;
    }
}
...
bool Config::ParseConfigFile(char const *const configfilename) {
    ...
    // LOG_MSG("CONFIG: Loading %s settings from config file %s",
    ...
    current_config_dir
        .clear(); // So internal changes don't use the path information
    return true;
}
...
bool CommandLine::FindHex(char const *const name, unsigned int &value,
                          bool remove) {
    cmd_it it, it_next;
...
bool CommandLine::FindString(char const *const name, std::string &value,
                             bool remove) {
    cmd_it it, it_next;
    if (!(FindEntry(name, it, true)))
        return false;
...
bool CommandLine::FindStringBegin(char const *const begin, std::string &value,
                                  bool remove) {
 ...
 * works as intended
 */
bool CommandLine::FindStringRemainBegin(char const *const name,
                                        std::string &value) {
    cmd_it it;
    value.clear();
    if (!FindEntry(name, it)) {
...
unsigned int CommandLine::GetCount(void) { return (unsigned int)cmds.size(); }
...
int CommandLine::GetParameterFromList(const char *const params[],
    ...
    return retval;
    /*
    bool CommandLine::FindEntry(char const * const name,cmd_it & it,bool
    neednext) { for (it=cmds.begin();it!=cmds.end();it++) { if
    (!strcasecmp((*it).c_str(),name)) { cmd_it itnext=it;itnext++; if (neednext
    && (itnext==cmds.end())) return false; return true;
                    }
            }
            return false;
    */
